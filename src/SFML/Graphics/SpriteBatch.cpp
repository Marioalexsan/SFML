                        ...
                        float            depth)
{
    ...
    switch (type)
    {
        ...
        default:
            assert((static_cast<void>("A non-triangle primitive type was encountered when decomposing into triangles."),
                    false));
            break;
    }
}
...
void SpriteBatch::draw(RenderTarget& target, const RenderStates& states) const
{
    ...
    {
        statesCopy.texture = m_batches[i].texture;

        target.draw(&m_vertices[startTriangle], m_batches[i].vertexCount, PrimitiveType::Triangles, statesCopy);

        startTriangle += m_batches[i].vertexCount;
    }
}
